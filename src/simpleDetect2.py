...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		if (RUNNING):
			#rate.sleep()
			cv_imageColor = bridge.imgmsg_to_cv2(IMAGE1)
			(scoresColor, boxesColor) = detect(cv_imageColor)
			detectionsColor = generateDetections(scoresColor, boxesColor, classes, args.treshold)
			if (pub_singleColor.get_num_connections() > 0):
				for msg in detectionsColor:
					pub_singleColor.publish(msg)		
			if (pub_arrayColor.get_num_connections() > 0 or pub_fullColor.get_num_connections() > 0):
				array = DetectionArray()
				array.size = len(detectionsColor)
				array.data = detectionsColor
				
				if (pub_fullColor.get_num_connections() > 0):
					msg = DetectionFull()
					msg.header.stamp = rospy.Time.now()
					msg.detections = array
					msg.image =  bridge.cv2_to_imgmsg(getResultImageColor(detectionsColor, cv_imageColor))
					pub_resultedColor.publish(msg.image)
					pub_fullColor.publish(msg)
				else :
					pub_arrayColor.publish(array)
			...
			if (pub_arrayThermal.get_num_connections() > 0 or pub_fullThermal.get_num_connections() > 0):
				...
				if (pub_fullThermal.get_num_connections() > 0):
					msg = DetectionFull()
					msg.detections = array
					msg.image =  bridge.cv2_to_imgmsg(getResultImageColor(detectionsThermal, cv_imageThermal))
